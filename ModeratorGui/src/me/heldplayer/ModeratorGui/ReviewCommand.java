	...
	public boolean onCommand(CommandSender sender, Command command, String alias, String[] args) {
		...
		if (args[1].equalsIgnoreCase("by")) {
			if (args.length == 2) {
				...
				int rowCount = main.getDatabase().find(Lists.class).where().eq("reporter", name).findRowCount();
				...
				int sideI = 3;
				for (int i = rowCount; i > (rowCount <= 10 ? 0 : rowCount - 10); i--) {
					Lists list = main.getDatabase().find(Lists.class).where().eq("id", i).eq("reporter", args[1]).findUnique();
		...
		if (args[1].equalsIgnoreCase("target")) {
			if (args.length == 2) {
				...
				int rowCount = main.getDatabase().find(Lists.class).where().eq("target", name).findRowCount();
				...
				int sideI = 3;
				for (int i = rowCount; i > (rowCount <= 10 ? 0 : rowCount - 10); i--) {
					Lists list = main.getDatabase().find(Lists.class).where().eq("id", i).eq("target", args[1]).findUnique();
